...
alpha_rate = 1e-2
#gamma_schedule = np.ones(num_steps)
gamma_schedule  = np.linspace(0, 1, num_steps)
#gamma_schedule  = np.tanh(5*(np.linspace(0,1,num_steps) - 1/2)) + 1
...
initial_alpha_arr = -mu_max * np.ones(num_spins)
...
# Print result
print(f"Best cut value: {int(np.max(cut_hist))}, optimum: {int(opt_cut)}")
print(f"Best energy: {int(np.min(energy_hist))}, optimum: {int(opt_energy)}")
