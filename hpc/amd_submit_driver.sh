...
#BSUB -n 110                     # total cores (max 128 for hpc, max 360 for fotonano)
#BSUB -R "span[hosts=1]"       # max cores/node (max 20 for hpc)
#BSUB -R "rusage[mem=2GB]"	# memory per core (2GB -> fast alloc)
#BSUB -W 10:00                  # max walltime (max 72 hours for hpc, max 120 hours for fotonano)
